...
const Navbar = memo(() => {
    ...
    return (
        <nav className="sticky top-0 z-50 backdrop-blur-xl bg-[#080810]/85 border-b border-[#6C5CE7]/20 shadow-[0_4px_30px_rgba(108,92,231,0.1)]">
            <div className="max-w-7xl mx-auto px-3 sm:px-4 py-3 flex items-center justify-between gap-3 sm:gap-4">
                ...
                >
                    ...
                    <div className="relative w-8 h-8 sm:w-9 sm:h-9 rounded-full bg-gradient-to-b from-[#ff4757] from-50% to-white to-50% border-[3px] border-[#2d2d44] shadow-[0_2px_15px_rgba(108,92,231,0.4)] transition-all duration-500 group-hover:rotate-[360deg] group-hover:shadow-[0_2px_20px_rgba(108,92,231,0.6)]">
                    ...
                    {/* Logo Text */}
                    <h1 className="font-extrabold text-lg sm:text-2xl tracking-tight transition-all duration-300">
                ...
                {/* Search Form - key fix: min-w-0 + flex-1 to prevent overflow */}
                <form
                    onSubmit={handleSearch}
                    className={`
                        relative flex items-center gap-1.5 sm:gap-2 rounded-xl px-2.5 sm:px-4 py-2 sm:py-2.5
                        flex-1 min-w-0 max-w-md
                        transition-all duration-300 ease-out
                        border backdrop-blur-md
                        ${isFocused
                            ? 'bg-[#6C5CE7]/15 border-[#6C5CE7]/50 shadow-[0_0_25px_rgba(108,92,231,0.25)]'
                ...
                >
                    ...
                    <input
                        ...
                        className="bg-transparent outline-none text-white placeholder:text-white/40 min-w-0 flex-1 text-sm font-medium"
                        aria-label="Search for a Pokémon"
                    />

                    {/* Suggestions Dropdown */}
                    {showsuggestions && suggestions.length > 0 && (
                        <div className="absolute top-full left-0 right-0 mt-2 bg-[#080810] border border-[#6C5CE7]/30 rounded-xl shadow-lg overflow-hidden z-50">
                    ...
                    <button
                        ...
                        className={`
                            px-2.5 sm:px-3 py-1.5 rounded-lg text-xs font-semibold flex-shrink-0
                            transition-all duration-300 ease-out
                            ${search.trim()
